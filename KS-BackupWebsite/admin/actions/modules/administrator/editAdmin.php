...
<form name="editpage" action="/admin/administrator/editAdminUpdate" method="post">
...
<table align="center">
   ...
   <?php for ($i=0; $i<count($adminGroupList); $i++)
   		{ 
		if ($adminGroupList[$i]['group_name'] != "") 
   			{?>
   <option value =" <?php echo $adminGroupList[$i]['id']; ?>"<?php if ($adminDetails[0]['admingroup']==$adminGroupList[$i]['id']) {echo " selected";} ?>><?php echo $adminGroupList[$i]['group_name'];?></option>
   <?php 	}
   		} ?>
   </select> 
   </td>
